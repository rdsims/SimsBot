...
public class JeVoisVisionServer
{
	...
	private void backgroundUpdate(){
		...
		if(packet != null)
		{
			// tie into Cheesy Poof VisionState and GoalTracker code
System.out.println("Got JSON, parsing...");			
			VisionState.getInstance().updateFromJsonString(timestamp, packet);
		}
		else
		{
    ...
    public void blockAndPrintAllSerial(){
        if (visionPort != null){
        	System.out.println("Starting Serial Output Display");
            while(serOutEnable){
                if (visionPort.getBytesReceived() > 0) {
